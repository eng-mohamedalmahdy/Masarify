...
fun stringResource(resource: StringResource?): String? =
    resource?.let { StringDesc.Resource(it) }?.localized()

@Composable
fun stringResource(resource: StringResource?, vararg args: Any): String? =
    resource?.let { StringDesc.ResourceFormatted(it, *args) }?.localized()

@Composable
fun stringResource(resource: PluralsResource?, quantity: Int): String? =
    resource?.let { StringDesc.Plural(it, quantity) }?.localized()

@Composable
fun stringResource(resource: PluralsResource?, quantity: Int, vararg args: Any): String? =
    resource?.let { StringDesc.PluralFormatted(it, quantity, *args) }?.localized()
